<template>
  <div class="sku-container">
    <div v-if="!disabled" class="sku-check">
      <div style="padding-bottom: 10px;width: 32%;">
        <el-input v-model="myAttr" size="small" placeholder="新增一个规格" @keyup.enter.native="onAddAttr()">
          <el-button slot="append" size="small" icon="el-icon-plus" @click="onAddAttr()">添加</el-button>
        </el-input>
      </div>
      ...
      <div v-else>
        <el-table v-if="myAttribute.length>0" :data="myAttribute" :show-header="false" class="theme-2">
          <el-table-column align="left" width="100" prop="name" :resizable="false" />
          <el-table-column align="left">
            <template slot-scope="scope">
              ...
              >
                <el-button slot="append" size="small" icon="el-icon-plus" @click="onAddAttribute(scope.$index)">添加
                </el-button>
              </el-input>
            </template>
          </el-table-column>
        ...
        <el-table :data="form.skuData" stripe border highlight-current-row>
          ...
          <template v-if="isBatch && form.skuData.length > 2" slot="append">
            <el-table :data="[{}]" :show-header="false">
              <el-table-column :width="attribute.length * 200 + 50" align="center" :resizable="false">批量设置
              </el-table-column>
              <el-table-column
                v-for="(item, index) in structure"
...
export default {
  ...
  methods: {
      ...
      if (index === 0) {
        ...
        for (let i = 0; i < dataTemp.length; i++) {
          for (let j = 0; j < this.attribute[index].item.length; j++) {
            ...
            temp[temp.length - 1]['sku'] = [temp[temp.length - 1]['sku'], this.attribute[index].item[j]].join(this
              .separator)
          }
        }
        dataTemp = temp
    ...
    // 新增一个规格
    onAddAttr() {
      this.myAttr = this.myAttr.trim()
      if (this.myAttr !== '') {
        if (!this.myAttr.includes(this.separator)) {
          const flag = this.myAttribute.some(item => {
            return item.name === this.myAttr
          })
          if (!flag) {
