    ...
    <el-row class="app-toptool" type="flex">
      ...
      <el-col :span="8">
        <div class="btn-group" style="margin-bottom:11px;float: right;">
          <div>
            <el-button
              v-if="checkPermission('/memberauthgroup/add')"
              type="primary"
              size="small"
              icon="el-icon-plus"
              @click="add()"
            >添加</el-button>
            <el-button
              v-if="checkPermission('/memberauthgroup/delete')"
              type="primary"
              :disabled="multiple"
              size="small"
              icon="el-icon-delete"
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <el-switch
            v-model="scope.row.is_buyright"
            :active-value="1"
            :inactive-value="0"
            @change="listUpdate(scope.row,'is_buyright')"
          />
        </template>
      </el-table-column>
      <el-table-column
        align="center"
        property="is_lookprice"
        label="能否查看商品价格"
        show-overflow-tooltip
        width=""
      >
        <template slot-scope="scope">
          <el-switch
            v-model="scope.row.is_lookprice"
            :active-value="1"
            :inactive-value="0"
            @change="listUpdate(scope.row,'is_lookprice')"
          />
        </template>
      </el-table-column>
      <el-table-column
      ...
      >
        <template slot-scope="scope">
          <div v-if="scope.row.id">
            <el-button
              v-if="checkPermission('/memberauthgroup/update')"
              size="mini"
              type="primary"
...
import Update from '@/views/memberauthgroup/update.vue'
...
export default {
  ...
  methods: {
    getList() {
      ...
      this.loading = true
      this.$api.post('/memberauthgroup/index', param).then(res => {
        this.list = res.data
        this.loading = false
    ...
    listUpdate(row, field) {
      if (row.id) {
        this.$api.post('/memberauthgroup/listUpdate', {
          id: row.id,
          [field]: row[field]
        }).then(res => {
          this.$message({
    ...
    update(row) {
      ...
      this.$api.post('/memberauthgroup/getInfo', {
        id: id
      }).then(res => {
        this.dialog.updateDialogStatus = true
    ...
    del(row) {
      confirm({
        ...
        this.$api.post('/memberauthgroup/delete', {
          id: ids
        }).then(res => {
          this.$message({
            message: res.msg,
