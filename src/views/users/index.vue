  ...
  <div>
    <el-row class="app-toptool" type="flex">
      ...
      <el-col :span="8">
        <div class="btn-group" style="margin-bottom:11px;float: right;">
          <div>
            <el-button
              v-if="checkPermission('/users/add')"
              type="primary"
              size="small"
              icon="el-icon-plus"
              @click="add()"
            ...
            <el-button
              v-if="checkPermission('/users/forbidden')"
              type="primary"
              :disabled="multiple"
              size="small"
              icon="el-icon-delete"
            ...
            <el-button
              v-if="checkPermission('/users/resetPwd')"
              type="primary"
              :disabled="single"
              size="small"
              icon="el-icon-lock"
    ...
    <div>
      ...
      >
        ...
        <el-table-column align="center" property="touxiang" label="头像" show-overflow-tooltip width="90">
        ...
        <el-table-column align="center" property="title" label="用户姓名" show-overflow-tooltip width="" />
        <el-table-column align="center" property="username" label="用户名" show-overflow-tooltip width="" />
        ...
        <el-table-column align="center" property="roles.title" label="所属角色" show-overflow-tooltip width="" />
        <el-table-column align="center" property="remark" label="备注" show-overflow-tooltip width="" />
        ...
        >
          <template slot-scope="scope">
            <div v-if="scope.row.id">
              <el-button v-if="checkPermission('/users/update')" size="mini" type="primary" @click="update(scope.row)">
    ...
    />
    <!--重置密码-->
    <ResetPwd :info="resetPwdInfo" :show.sync="dialog.resetPwdDialogStatus" size="small" @refesh_list="index" />
  </div>
</template>
<script>
...
import ResetPwd from '@/views/users/resetPwd.vue'
import {
  confirm,
  param2Obj
} from '@/utils/common'
export default {
  ...
  methods: {
    ...
    index() {
      ...
      this.$api.post('/Users/index', param).then(res => {
    ...
    listUpdate(row, field) {
      if (row.id) {
        this.$api.post('/Users/listUpdate', {
          id: row.id,
          [field]: row[field]
        }).then(res => {
          this.$message({
    ...
    resetPwd(row) {
      ...
      this.resetPwdInfo = {
        id: row.id ? row.id : this.ids.join(',')
      }
    },
    forbidden(row) {
      confirm({
