    ...
    <el-row class="app-toptool" type="flex">
      ...
      <el-col :span="8">
        <div class="btn-group" style="margin-bottom:11px;float: right;">
          <div>
            <el-button
              v-if="checkPermission('/operatingcity/add')"
              type="primary"
              size="small"
              icon="el-icon-plus"
              @click="add()"
            >添加</el-button>
            <el-button
              v-if="checkPermission('/operatingcity/delete')"
              type="primary"
              :disabled="multiple"
              size="small"
              icon="el-icon-delete"
        ...
        >
          <template slot-scope="scope">
            <div v-if="scope.row.id">
              <el-button
                v-if="checkPermission('/operatingcity/login')"
                size="mini"
                type="primary"
              ...
              <el-button
                v-if="checkPermission('/operatingcity/update')"
                size="mini"
                type="primary"
...
export default {
  ...
  methods: {
    ...
    login(row) {
      const id = row.id ? row.id : this.ids.join(',')
      window.open('?ocid=' + id + '#/dashboard', '_blank')
    },
    del(row) {
      confirm({
        content: '确定要操作吗'
        ...
        this.$api.post('/operatingcity/delete', {
          path: this.$route.path,
          id: ids
        }).then(res => {
          this.$message({
