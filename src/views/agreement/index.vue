        ...
        >
          <template slot-scope="scope">
            <div v-if="scope.row.id">
              <el-button
                v-if="checkPermission('/agreement/update')"
                size="mini"
                type="primary"
...
export default {
  ...
  methods: {
    ...
    index() {
      ...
      this.$api.post('/agreement/index', param).then(res => {
    ...
    listUpdate(row, field) {
      if (row.id) {
        this.$api.post('/agreement/listUpdate', {
          id: row.id,
          [field]: row[field]
        }).then(res => {
          this.$message({
    ...
    update(row) {
      ...
      this.$api.post('/agreement/getInfo', {
        id: id
      }).then(res => {
        this.dialog.updateDialogStatus = true
    ...
    del(row) {
      confirm({
        ...
        this.$api.post('/agreement/delete', {
          id: ids
        }).then(res => {
          this.$message({
            message: res.msg,
