<template>
  <div>
    <el-row class="app-toptool" type="flex">
      ...
      <el-col :span="8">
        <div class="btn-group" style="margin-bottom:11px;float: right;">
          <div>
            <el-button
              v-if="checkPermission('/diypage/add')"
              type="primary"
              size="small"
              icon="el-icon-plus"
              @click="add()"
            >添加</el-button>
            <el-button
              v-if="checkPermission('/diypage/delete')"
              type="primary"
              :disabled="multiple"
              size="small"
              icon="el-icon-delete"
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <el-switch
              v-model="scope.row.is_submitaudit"
              :active-value="1"
              :inactive-value="0"
              @change="listUpdate(scope.row,'is_submitaudit')"
            />
          </template>
        ...
        >
          <template slot-scope="scope">
            <el-switch
              ...
              :active-value="1"
              :inactive-value="0"
              @change="listUpdate(scope.row,'is_index')"
            />
          </template>
        ...
        >
          <template slot-scope="scope">
            <div v-if="scope.row.id">
              <el-button
                v-if="checkPermission('/diypage/update')"
                size="mini"
                type="primary"
    ...
    <!--添加-->
    <Add :info="updateInfo" :opentype="opentype" :show.sync="dialog.addDialogStatus" size="small" @refesh_list="index" />
    <!--修改-->
...
import Add from '@/views/diypage/add.vue'
import Update from '@/views/diypage/update.vue'
import {
  confirm,
  param2Obj
} from '@/utils/common'
export default {
  ...
  methods: {
    index() {
      ...
      this.$api.post('/diypage/index', param).then(res => {
    ...
    listUpdate(row, field) {
      if (row.id) {
        this.$api.post('/diypage/listUpdate', {
          id: row.id,
          [field]: row[field]
        }).then(res => {
          this.$message({
