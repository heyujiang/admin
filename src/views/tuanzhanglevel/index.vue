    ...
    <el-row class="app-toptool" type="flex">
      ...
      <el-col :span="8">
        <div class="btn-group" style="margin-bottom:11px;float: right;">
          <div>
            <el-button
              v-if="checkPermission('/tuanzhanglevel/add')"
              type="primary"
              size="small"
              icon="el-icon-plus"
              @click="add()"
            >添加</el-button>
            <el-button
              v-if="checkPermission('/tuanzhanglevel/delete')"
              type="primary"
              :disabled="multiple"
              size="small"
              icon="el-icon-delete"
      ...
      >
        <template slot-scope="scope">
          <div v-if="scope.row.id">
            <el-button
              v-if="checkPermission('/tuanzhanglevel/update')"
              size="mini"
              type="primary"
...
import Update from '@/views/tuanzhanglevel/update.vue'
import {
  confirm,
  param2Obj
} from '@/utils/common'
export default {
  ...
  methods: {
    getList() {
      ...
      this.loading = true
      this.$api.post('/tuanzhanglevel/index', param).then(res => {
        this.list = res.data
        this.loading = false
    ...
    listUpdate(row, field) {
      if (row.id) {
        this.$api.post('/tuanzhanglevel/listUpdate', {
          id: row.id,
          [field]: row[field]
        }).then(res => {
          this.$message({
    ...
    update(row) {
      ...
      this.$api.post('/tuanzhanglevel/getInfo', {
        id: id
      }).then(res => {
        this.dialog.updateDialogStatus = true
    ...
    del(row) {
      confirm({
        ...
        this.$api.post('/tuanzhanglevel/delete', {
          id: ids
        }).then(res => {
          this.$message({
            message: res.msg,
