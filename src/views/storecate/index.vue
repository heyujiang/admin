<template>
  <div>
    <el-row class="app-toptool" type="flex">
      ...
      <el-col :span="8">
        <div class="btn-group" style="margin-bottom:11px;float: right;">
          <div>
            <el-button v-if="checkPermission('/storecate/add')" type="primary" size="small" icon="el-icon-plus"
              @click="add()">添加</el-button>
            <el-button v-if="checkPermission('/storecate/delete')" type="primary" :disabled="multiple" size="small"
    ...
    <el-table ref="multipleTable" v-loading="loading" :row-class-name="rowClass" :tree-props="{children: 'children'}"
      ...
      <el-table-column align="left" property="return_percent" label="佣金比例%" show-overflow-tooltip width="" />
      <el-table-column align="left" property="create_time" label="创建时间" show-overflow-tooltip width="" />
      ...
      <el-table-column label="操作" align="center" width="100">
        <template slot-scope="scope">
          <div v-if="scope.row.id">
            <el-button v-if="checkPermission('/storecate/update')" size="mini" type="primary"
...
<script>
  ...
  import Update from '@/views/storecate/update.vue'
  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        this.loading = true
        this.$api.post('/storecate/index', param).then(res => {
          this.list = res.data
          this.loading = false
      ...
      listUpdate(row, field) {
        if (row.id) {
          this.$api.post('/storecate/listUpdate', {
            id: row.id,
            [field]: row[field]
          }).then(res => {
            this.$message({
      ...
      update(row) {
        ...
        this.$api.post('/storecate/getInfo', {
          id: id
        }).then(res => {
          this.dialog.updateDialogStatus = true
      ...
      del(row) {
        confirm({
          ...
          this.$api.post('/storecate/delete', {
            id: ids
          }).then(res => {
            this.$message({
              message: res.msg,
