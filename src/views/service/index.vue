<template>
  <div>
    <el-row class="app-toptool" type="flex">
      ...
      <el-col :span="6">
        <div class="btn-group" style="margin-bottom:11px;float: right;">
          <div>
            <el-button
              v-if="checkPermission('/service/add')"
              type="primary"
              size="small"
              icon="el-icon-plus"
              @click="add()"
    ...
    <Replace
      :info="goodsInfo"
      :show.sync="dialog.replaceDialogStatus"
      size="small"
    />
  </div>
</template>
...
import Update from '@/views/service/update.vue'
import Replace from '@/views/service/replace.vue'
import {
  confirm,
  param2Obj
} from '@/utils/common'
  ...
  methods: {
    ...
    replaceOrder(row) {
      ...
      this.$api.post('/goods/getInfo', {
        ...
        this.dialog.replaceDialogStatus = true        
      })
    },
    del(row) {
      confirm({
