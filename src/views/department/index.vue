    ...
    <el-row class="app-toptool" type="flex">
      ...
      <el-col :span="8">
        <div class="btn-group" style="margin-bottom:11px;float: right;">
          <div>
            <el-button
              v-if="checkPermission('/department/add')"
              type="primary"
              size="small"
              icon="el-icon-plus"
              @click="add()"
...
import Update from '@/views/department/update.vue'
...
export default {
  ...
  methods: {
    getList() {
      ...
      this.loading = true
      this.$api.post('/Department/index', param).then(res => {
        this.list = res.data
        this.loading = false
    ...
    listUpdate(row, field) {
      if (row.id) {
        this.$api.post('/Department/listUpdate', {
          id: row.id,
          [field]: row[field]
        }).then(res => {
          this.$message({
    ...
    update(row) {
      ...
      this.$api.post('/Department/getInfo', {
        id: id
      }).then(res => {
        this.dialog.updateDialogStatus = true
    ...
    del(row) {
      confirm({
        ...
        this.$api.post('/Department/delete', {
          id: ids
        }).then(res => {
          this.$message({
            message: res.msg,
