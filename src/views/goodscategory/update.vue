          ...
          <el-col :span="24">
            <el-form-item label="交付方式">
              <el-checkbox-group v-model="form.deliverymode">
                <el-checkbox v-for="(item,i) in goodsdeliverymodearray" :key="i" :label="item.val.toString()">
                  {{ item.key }}
                </el-checkbox>
              </el-checkbox-group>
            </el-form-item>
    ...
    show(val) {
      if (val) {
        this.$api.post('/Category/getField', {
          ...
          this.goodsdeliverymodearray = res.data.goodsdeliverymodearray
        })
      }
    }
  },
