    ...
    <el-row class="app-toptool" type="flex">
      ...
      <el-col :span="8">
        <div class="btn-group" style="margin-bottom:11px;float: right;">
          <div>
            <el-button
              v-if="checkPermission('/printer/add')"
              type="primary"
              size="small"
              icon="el-icon-plus"
              @click="add()"
            >添加</el-button>
            <el-button
              v-if="checkPermission('/printer/delete')"
              type="primary"
              :disabled="multiple"
              size="small"
              icon="el-icon-delete"
        ...
        >
          <template slot-scope="scope">
            <div v-if="scope.row.id">
              <el-button
                v-if="checkPermission('/printer/update')"
                size="mini"
                type="primary"
...
import Update from '@/views/printer/update.vue'
import {
  confirm,
  param2Obj
} from '@/utils/common'
export default {
  ...
  methods: {
    ...
    index() {
      ...
      this.$api.post('/printer/index', param).then(res => {
    ...
    listUpdate(row, field) {
      if (row.id) {
        this.$api.post('/printer/listUpdate', {
          id: row.id,
          [field]: row[field]
        }).then(res => {
          this.$message({
