  ...
  <div>
    <el-row class="app-toptool" type="flex">
      ...
      <el-col :span="8">
        <div class="btn-group" style="margin-bottom:11px;float: right;">
          <div>
            <el-button v-if="checkPermission('/member/delete')" type="primary" :disabled="multiple" size="small"
    ...
    <div>
      <el-table ref="multipleTable" v-loading="loading" :row-class-name="rowClass" row-key="id"
        ...
        <el-table-column v-for="column in RegisterField" :key="column.fieldsmingcheng"
          :prop="column.inputtype+','+column.fieldsmingcheng" :label="column.viewmingcheng" show-overflow-tooltip>
          <template slot-scope="scope">
            <div v-if="scope.column.property.split(',')[0] == 'pics'">
              <div class="demo-image__preview">
                <el-image class="table_list_pic"
        ...
        <el-table-column :fixed="$store.getters.device !== 'mobile'?'right':false" label="操作" align="center"
          ...
          <template slot-scope="scope">
            <div v-if="scope.row.id">
              <el-button type="primary" plain size="mini" @click="goserviceorder(scope.row)">查看服务订单
              </el-button>
              <el-button type="primary" plain size="mini" @click="gogoodsorder(scope.row)">查看商品订单
              </el-button>
              <el-button v-if="checkPermission('/member/update')" size="mini" type="primary"
