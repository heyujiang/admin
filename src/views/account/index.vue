    ...
    <el-row class="app-toptool" type="flex">
      ...
      <el-col :span="8">
        <div class="btn-group" style="margin-bottom:11px;float: right;">
          <div>
            <el-button
              v-if="checkPermission('/account/add')"
              type="primary"
              size="small"
              icon="el-icon-plus"
              @click="add()"
            >添加</el-button>
            <el-button
              v-if="checkPermission('/account/delete')"
              type="primary"
              :disabled="multiple"
              size="small"
              icon="el-icon-delete"
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <div class="demo-image__preview">
              <el-image
                v-if="scope.row.loginbgimg"
                class="table_list_pic"
                :src="scope.row.loginbgimg"
                :preview-src-list="[scope.row.loginbgimg]"
              />
            </div>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    index() {
      ...
      this.$api.post('/account/index', param).then(res => {
    ...
    listUpdate(row, field) {
      if (row.id) {
        this.$api.post('/account/listUpdate', {
          id: row.id,
          [field]: row[field]
        }).then(res => {
          this.$message({
    ...
    goaccount(row) {
      const id = row.id ? row.id : this.ids.join(',')
      window.open('?i=' + id + '#/dashboard', '_blank')
    },
    del(row) {
      confirm({
        content: '确定要操作吗'
