    ...
    <el-row class="app-toptool" type="flex">
      ...
      <el-col :span="8">
        <div class="btn-group" style="margin-bottom:11px;float: right;">
          <div>
            <el-button
              v-if="checkPermission('/articlecategory/add')"
              type="primary"
              size="small"
              icon="el-icon-plus"
              @click="add()"
            >添加</el-button>
            <el-button
              v-if="checkPermission('/articlecategory/delete')"
              type="primary"
              :disabled="multiple"
              size="small"
              icon="el-icon-delete"
      ...
      >
        <template slot-scope="scope">
          <div v-if="scope.row.id">
            <el-button
              v-if="checkPermission('/articlecategory/update')"
              size="mini"
              type="primary"
...
import Update from '@/views/articlecategory/update.vue'
...
export default {
  ...
  methods: {
    getList() {
      ...
      this.loading = true
      this.$api.post('/articlecategory/index', param).then(res => {
        this.list = res.data
        this.loading = false
    ...
    listUpdate(row, field) {
      if (row.id) {
        this.$api.post('/articlecategory/listUpdate', {
          id: row.id,
          [field]: row[field]
        }).then(res => {
          this.$message({
    ...
    update(row) {
      ...
      this.$api.post('/articlecategory/getInfo', {
        id: id
      }).then(res => {
        this.dialog.updateDialogStatus = true
    ...
    del(row) {
      confirm({
        ...
        this.$api.post('/articlecategory/delete', {
          id: ids
        }).then(res => {
          this.$message({
            message: res.msg,
