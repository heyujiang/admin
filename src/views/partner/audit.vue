    ...
    <el-row class="app-toptool" type="flex">
      ...
      <el-col :span="8">
        <div class="btn-group" style="margin-bottom:11px;float: right;">
          <div>
            <el-button
              v-if="checkPermission('/partner/delete')"
              type="primary"
              :disabled="multiple"
              size="small"
              icon="el-icon-delete"
        ...
        >
          <template slot-scope="scope">
            <div v-if="scope.row.id">
              <el-button v-if="checkPermission('/partner/auditdetail')" size="mini" type="primary" @click="audit(scope.row)"><i
...
export default {
  ...
  methods: {
    ...
    index() {
      ...
      this.$api.post('/partner/index', param).then(res => {
    ...
    listUpdate(row, field) {
      if (row.id) {
        this.$api.post('/partner/listUpdate', {
          path: this.$route.path,
          id: row.id,
          [field]: row[field]
        }).then(res => {
