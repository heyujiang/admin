  ...
  <div>
    <el-row class="app-toptool" type="flex">
      ...
      <el-col :span="6">
        <div class="btn-group" style="margin-bottom:11px;float: right;">
          <div>
            <el-button
              v-if="checkPermission('/timecard/add')"
              type="primary"
              size="small"
              icon="el-icon-plus"
              @click="add()"
            >添加</el-button>
            <el-button
              v-if="checkPermission('/timecard/delete')"
              type="primary"
              :disabled="multiple"
              size="small"
              icon="el-icon-delete"
    ...
    <div>
      ...
      >
        ...
        <el-table-column align="left" property="timesmum" label="次数" show-overflow-tooltip width="" />
        <el-table-column align="center" property="is_timing" label="定期服务" show-overflow-tooltip width="">
          <template slot-scope="scope">
            <el-switch
              v-model="scope.row.is_timing"
              :active-value="1"
              :inactive-value="0"
              @change="listUpdate(scope.row,'is_timing')"
            />
          </template>
        </el-table-column>
        ...
        <el-table-column align="center" property="is_additional" label="支持尾款" show-overflow-tooltip width="">
          <template slot-scope="scope">
            <el-switch
              v-model="scope.row.is_additional"
              :active-value="1"
              :inactive-value="0"
              @change="listUpdate(scope.row,'is_additional')"
            />
          </template>
        </el-table-column>
        ...
        >
          <template slot-scope="scope">
            <div v-if="scope.row.id">
              <el-button
                v-if="checkPermission('/timecard/update')"
                size="mini"
                type="primary"
...
import Update from '@/views/timecard/update.vue'
import {
  confirm,
  param2Obj
} from '@/utils/common'
