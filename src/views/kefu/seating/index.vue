  ...
  <div>
    <el-row class="app-toptool" type="flex">
      ...
      <el-col :span="8">
        <div class="btn-group" style="margin-bottom:11px;float: right;">
          <div>
            <el-button
              v-if="checkPermission('/kefu/seating/add')"
              type="success"
              size="small"
              icon="el-icon-plus"
              @click="add()"
            >添加</el-button>
            <el-button
              v-if="checkPermission('/kefu/seating/delete')"
              type="primary"
              :disabled="multiple"
              size="small"
              icon="el-icon-delete"
    ...
    <div id="charts_one" style="width:100%;min-height:80vh">
      ...
      >
        ...
        <el-table-column align="center" property="title" label="名称" show-overflow-tooltip width="" />
        <el-table-column align="center" property="touxiang" label="图标" show-overflow-tooltip width="90">
        ...
        >
          <template slot-scope="scope">
            <div v-if="scope.row.id">
              <el-button v-if="checkPermission('/kefu/seating/update')" size="mini" type="primary" @click="update(scope.row)"><i
                class="el-icon-edit"
              />修改</el-button>
            </div>
          </template>
...
import Update from '@/views/kefu/seating/update.vue'
import {
  confirm,
  param2Obj
} from '@/utils/common'
export default {
  ...
  methods: {
    index() {
      ...
      this.$api.post('/kefu.Seating/index', param).then(res => {
    ...
    listUpdate(row, field) {
      if (row.id) {
        this.$api.post('/kefu.Seating/listUpdate', {
          id: row.id,
          [field]: row[field]
        }).then(res => {
          this.$message({
    ...
    update(row) {
      ...
      this.$api.post('/kefu.Seating/getInfo', {
        id: id
      }).then(res => {
        this.dialog.updateDialogStatus = true
    ...
    del(row) {
      confirm({
        ...
        this.$api.post('/kefu.Seating/delete', {
          id: ids
        }).then(res => {
          this.$message({
            message: res.msg,
...
<style>
  .table_list_pic {width:60px; height:60px; vertical-align: middle;}
</style>
