    ...
    <el-row class="app-toptool" type="flex">
      ...
      <el-col :span="8">
        <div class="btn-group" style="margin-bottom:11px;float: right;">
          <div>
            <el-button
              v-if="checkPermission('/kefu/seatinggroups/add')"
              type="primary"
              size="small"
              icon="el-icon-plus"
              @click="add()"
            >添加</el-button>
            <el-button
              v-if="checkPermission('/kefu/seatinggroups/delete')"
              type="primary"
              :disabled="multiple"
              size="small"
              icon="el-icon-delete"
        ...
        >
          <template slot-scope="scope">
            <div v-if="scope.row.id">
              <el-button v-if="checkPermission('/kefu/seatinggroups/update')" size="mini" type="primary" @click="update(scope.row)"><i
                class="el-icon-edit"
              />修改</el-button>
            </div>
          </template>
...
import Update from '@/views/kefu/seatinggroups/update.vue'
import {
  confirm,
  param2Obj
} from '@/utils/common'
export default {
  ...
  methods: {
    index() {
      ...
      this.$api.post('/kefu.Seatinggroups/index', param).then(res => {
    ...
    listUpdate(row, field) {
      if (row.id) {
        this.$api.post('/kefu.Seatinggroups/listUpdate', {
          id: row.id,
          [field]: row[field]
        }).then(res => {
          this.$message({
    ...
    update(row) {
      ...
      this.$api.post('/kefu.Seatinggroups/getInfo', {
        id: id
      }).then(res => {
        this.dialog.updateDialogStatus = true
    ...
    del(row) {
      confirm({
        ...
        this.$api.post('/kefu.Seatinggroups/delete', {
          id: ids
        }).then(res => {
          this.$message({
            message: res.msg,
