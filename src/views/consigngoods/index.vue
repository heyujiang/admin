    ...
    <el-row class="app-toptool" type="flex">
      ...
      <el-col :span="8">
        <div class="btn-group" style="margin-bottom:11px;float: right;">
          <div>
            <el-button
              v-if="checkPermission('/consigngoods/add')"
              type="primary"
              size="small"
              icon="el-icon-plus"
              @click="add()"
            >添加</el-button>
            <el-button
              v-if="checkPermission('/consigngoods/delete')"
              type="primary"
              :disabled="multiple"
              size="small"
              icon="el-icon-delete"
      ...
      >
        ...
        <el-table-column align="center" property="image" label="图片" show-overflow-tooltip width="90">
          <template slot-scope="scope">
            <div>
              <el-image v-if="scope.row.image" class="table_list_pic" :src="scope.row.image"
        ...
        >
          <template slot-scope="scope">
            <div v-if="scope.row.id">
              <el-button
                v-if="checkPermission('/consigngoods/update')"
                size="mini"
                type="primary"
...
import Update from '@/views/consigngoods/update.vue'
import {
  confirm,
  param2Obj
} from '@/utils/common'
export default {
  ...
  methods: {
    ...
    index() {
      ...
      this.$api.post('/consigngoods/index', param).then(res => {
    ...
    listUpdate(row, field) {
      if (row.id) {
        this.$api.post('/consigngoods/listUpdate', {
          id: row.id,
          [field]: row[field]
        }).then(res => {
          this.$message({
    ...
    update(row) {
      ...
      this.$api.post('/consigngoods/getInfo', {
        id: id
      }).then(res => {
        this.dialog.updateDialogStatus = true
    ...
    del(row) {
      confirm({
        ...
        this.$api.post('/consigngoods/delete', {
          id: ids
        }).then(res => {
          this.$message({
            message: res.msg,
