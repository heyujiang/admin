<template>
  <div>
    <el-row class="app-toptool" type="flex">
      ...
      <el-col :span="8">
        <div class="btn-group" style="margin-bottom:11px;float: right;">
          <div>
            <el-button
              v-if="checkPermission('/tuanzhang/delete')"
              type="primary"
              :disabled="multiple"
              size="small"
              icon="el-icon-delete"
            ...
            <el-button
              v-if="checkPermission('/tuanzhang/importData')"
              type="primary"
              size="small"
              icon="el-icon-upload"
            ...
            >导入</el-button>
            <el-button
              v-if="checkPermission('/tuanzhang/dumpdata')"
              type="primary"
              size="small"
        ...
        >
          <template slot-scope="scope">
            <div v-if="scope.row.id">
              <el-button v-if="checkPermission('/tuanzhang/auditdetail')" size="mini" type="primary" @click="audit(scope.row)"><i
    ...
    />
    <!--导入-->
    <ImportData :show.sync="dialog.importDataDialogStatus" controller="tuanzhang" size="small" @refesh_list="index" />
...
export default {
  ...
  methods: {
    ...
    index() {
      ...
      this.$api.post('/tuanzhang/index', param).then(res => {
    ...
    listUpdate(row, field) {
      if (row.id) {
        this.$api.post('/tuanzhang/listUpdate', {
          path: this.$route.path,
          id: row.id,
          [field]: row[field]
        }).then(res => {
    ...
    del(row) {
      confirm({
        ...
        this.$api.post('/tuanzhang/delete', {
          path: this.$route.path,
          id: ids
        }).then(res => {
          this.$message({
