        ...
        >
          <template slot-scope="scope">
            <div v-if="scope.row.id">
              <el-button
                v-if="checkPermission('/ossupload/update')"
                size="mini"
                type="primary"
...
import Update from '@/views/ossupload/update.vue'
import {
  confirm,
  param2Obj
} from '@/utils/common'
export default {
  ...
  methods: {
    ...
    index() {
      ...
      this.$api.post('/ossupload/index', param).then(res => {
    ...
    listUpdate(row, field) {
      if (row.id) {
        this.$api.post('/ossupload/listUpdate', {
          id: row.id,
          [field]: row[field]
        }).then(res => {
          this.$message({
    ...
    update(row) {
      ...
      this.$api.post('/ossupload/getInfo', {
        id: id
      }).then(res => {
    ...
    del(row) {
      confirm({
        ...
        this.$api.post('/ossupload/delete', {
          id: ids
        }).then(res => {
          this.$message({
            message: res.msg,
